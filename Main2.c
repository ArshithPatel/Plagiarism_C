...
int main(int argc, char const *argv[])
{
	...
	for (int i=0; i<n; i++)
	{
		...
		for (int j=0; j<n; j++)
		{
			if (i==j)
			{
				...
				printf("\t%0.2f", BOW_Arr[i][j]);		
			}
			else
			{
				BOW_Arr[i][j] = BOW(filesList[i],filesList[j]);		//Calling BagOfWords functionality for two files in a loop.
				printf("\t%0.2f", BOW_Arr[i][j]);
			}
	...
	for (int i=0; i<n; i++)
	{
		...
		for (int j=0; j<n; j++)
		{
			if (i==j)
			{
				...
				printf("\t%0.2f", LCS_Arr[i][j]);
			}
			else
			{
				LCS_Arr[i][j] = LongestCommonSubstring(filesList[i],filesList[j]);		//Calling Longest Common Substring for two files in a loop
				printf("\t%0.2f", LCS_Arr[i][j]);
			}
	...
	for (int i=0; i<n; i++)
	{
		...
		for (int j=0; j<n; j++)
		{
			...
			{
				F_Print[i][j] = Finger_Printing(filesList[i],filesList[j]);	//Calling Finger Printing for two files in a loop
				printf("\t%0.2f", F_Print[i][j]);
			}
